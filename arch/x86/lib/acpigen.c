...
static int acpigen_write_field_offset(uint32_t offset, uint32_t current_bit_pos)
{
	uint32_t diff_bits;
 ...
 *	}
 */
int acpigen_write_field(const char *name, struct fieldlist *l, size_t count,
			uint8_t flags)
{
	uint16_t i;
	...
	for (i = 0; i < count; i++) {
		...
		case NAME_STRING:
			...
			break;
		case OFFSET:
			ret = acpigen_write_field_offset(l[i].bits, current_bit_pos);
			if (ret)
 ...
 *	}
 */
int acpigen_write_indexfield(const char *idx, const char *data,
...
static void acpigen_write_register(const struct acpi_gen_regaddr *addr)
{
 ...
 */
#define UUID_LEN 16
int acpigen_write_uuid(const char *uuid)
{
...
int acpigen_write_dsm(const char *uuid, void (**callbacks)(void *),
		      size_t count, void *arg)
{
	struct dsm_uuid id = DSM_UUID(uuid, callbacks, count, arg);
	int ret;

	ret = acpigen_write_dsm_uuid_arr(&id, 1);
	if (ret)
...
static int acpigen_write_dsm_uuid(struct dsm_uuid *id)
{
	size_t i;
	int ret;
	...
	ret = acpigen_write_uuid(id->uuid);
	if (ret)
 ...
 * }
 */
int acpigen_write_dsm_uuid_arr(struct dsm_uuid *ids, size_t count)
{
	size_t i;
	int ret;
	...
	for (i = 0; i < count; i++) {
		ret = acpigen_write_dsm_uuid(&ids[i]);
		if (ret)
			return ret;
	}
...
int acpigen_write_cppc_package(const struct cppc_config *config)
{
	u32 i;
	u32 max;
...
int acpigen_write_rom(void *bios, const size_t length)
{
	int ret;
	 ...
	 *  RBF0,   0x80000
	 * } */
	ret = acpigen_write_field(opreg.name, l, 2, FIELD_ANYACC |
