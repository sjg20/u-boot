...
#if CONFIG(MIPS_BOOT_FDT) && CONFIG(OF_LIBFDT)
...
{
	if (CONFIG(MIPS_BOOT_FDT) && images->ft_len) {
		boot_reloc_fdt(images);
		boot_setup_fdt(images);
	} else {
		if (CONFIG(MIPS_BOOT_CMDLINE_LEGACY)) {
			linux_cmdline_legacy(images);

			if (!CONFIG(MIPS_BOOT_ENV_LEGACY))
...
#if CONFIG(BOOTSTAGE_FDT)
	bootstage_fdt_add_report();
#endif
#if CONFIG(BOOTSTAGE_REPORT)
	bootstage_report();
#endif

	if (CONFIG(RESTORE_EXCEPTION_VECTOR_BASE))
		trap_restore();
