...
so avoiding the need for the `device_bind()` code.
...
The `plat_` mmember is set to the dtv data which is declared immediately above
...
`.plat_auto` field, then a simple dtv struct can be usedm as with this example::
...
its own, besides that provided by the device tree (i.e. not `.plat_auto` field).
...
This is handled automatically by dtoc::
...
The other driver has the same name but no DM_PHASE(), so it will be used for
SPL/U-Boot.

Note that this only affects the code generated by dtoc. You still need to make
sure that only the required driver is build into each phase.
...
caveats include:
   ...
   - It is not possible to find a value given a property name. Code must use
     ...
     the code less robust in the face of device-tree changes. To avoid having
   ...
   - With CONFIG_OF_PLATDATA_INST all binding happens at build-time, meaning
     ...
     the child devices. It is possible to enable OF_PLATDATA_NO_BIND but this
     is not recommended.
...
OF_PLATDATA
   This is the main option which controls the of-platdata feature

OF_PLATDATA_PARENT
   This allows `device_get_parent()` to work. Without this, all devices exist as
   direct children of the root node. This is highly desirable (if not always
   absolutely essential) for buses such as I2C.
...
OF_PLATDATA_NO_BIND
   This controls whether `device_bind()` is supported. It is disabled by default
   with OF_PLATDATA_INST since code-size reduction is really the main point of
   the feature. It can be enabled if needed but is not likely to be supported in
   the long term.
...
OF_PLATDATA_DRIVER_RT
   ...
   `device_get_by_ofplat_idx()`.. This is always enabled with of-platdata,
...
`struct driver_rt`
   ...
   OF_PLATDATA_DRIVER_RT is enabled it, this holds a pointer to the device
   ...
   get a reference to the `struct driver_info`, with `name` being the name of
   the devicetree node. This is very convenient. It is also fast, since no
   searching or string comparison is needed. This data structure is allocated
   on start-up, filled out by `device_bind()` and used by
...
Accessor functions
   ...
   the read-write copy made on start-up. Changing the private / platform data
   pointers has always been discouraged (the API is marked internal) but with
   OF_PLATDATA_RT this is not currently supported in general, since it assumes
   that all such pointers point to the relocated data. Note also that the
   renaming of struct members to have a trailing underscore was partly done to
   make people aware that they should not be accessed directly.
...
`gd->dm_driver_rt`
   ...
   required #ifdefs in the C code when it is not present.
...
`gd->dm_udevice_rt`
   ...
   used inside IS_ENABLED(), rather than required #ifdefs in the C code when it
   is not present.
...
`struct udevice->flags_`
   When OF_PLATDATA_RT is enabled, this is no-longer part of `struct udevice`,
   but is instead kept in `struct udevice_rt`, as described above. Flags are
   accessed via functions, such as `dev_get_flags()` and `dev_or_flags()`.
...
`device_get_by_ofplat_idx()`
   ...
   looked up a `struct driver_info` pointer and returns the `struct udevice`
...
Two-pass binding
   ...
   not work the idx value relies on using alphabetical order for everything,
...
Root device
   ...
   (when OF_PLATDATA_INST is disabled) and the ix numbers used by
   `struct udevice` (when OF_PLATDATA_INST is enabled) differ. This does not
   actually matter, since only one of them is actually used for any particular
   build, but it is worth keeping in mind if comparing index values and
   switching OF_PLATDATA_INST on and off.
