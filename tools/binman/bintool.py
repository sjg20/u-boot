...
class Bintool:
    ...
    def run_cmd_result(self, *args, binary=False, raise_on_error=True):
        ...
        Returns:
            str or bytes: Resulting output from the bintool
        """
        if not self.is_present() or self.name in self.missing_list:
            return None
        return tools.run_result(self.name, *args, raise_on_error=raise_on_error,
                                binary=binary)

    def run_cmd(self, *args, binary=False, raise_on_error=True):
        """Run the bintool using command-line arguments
        ...
        Returns:
            str or bytes: Resulting output from the bintool
        """
        result = self.run_cmd_result(*args, raise_on_error=raise_on_error,
                                     binary=binary)
        if result:
    ...
    def WriteDocs(modules, test_missing=None):
        ...
        Args:
            ...
            test_missing: Used for testing. This is a module to report
                as missing
        """
        print('''Binman bintool Documentation
============================
