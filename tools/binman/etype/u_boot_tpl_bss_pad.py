...
class Entry_u_boot_tpl_bss_pad(Entry_blob):
    ...
    This is similar to u_boot_tpl except that padding is added after the TPL
    binary to cover the BSS (Block Started by Symbol) region. This region holds
    the various used by TPL. It is set to 0 by TPL when it starts up. If you
    want to append data to the TPL image (such as a device tree file), you must
