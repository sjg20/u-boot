...
class TestFunctional(unittest.TestCase):
    ed = b'Lord Edmund Blackadd\xc3\xabr <weasel@blackadder.org'.decode('utf-8')
    ...
    def testBasic(self):
        ...
        self.assertEqual('      Cc:  %s' % tools.FromUnicode(self.ed),
                         next(lines))
        ...
        self.assertEqual('%s %s\0%s\0%s\0%s' %
                         (args[1], self.fred, self.ed, rick, stefan),
...
'''
    ...
    def testNoUpstream(self):
        ...
        try:
            ...
            self.assertIn("Failed to determine upstream: fatal: no upstream configured for branch 'base'",
                          str(e.exception))
        finally:
            os.chdir(orig_dir)
    ...
    def testStatusMismatch(self):
        ...
        with capture_sys_output() as (out, err):
            status.collect_patches(series, 1234, self._FakePatchwork)
    ...
    def testStatusReadPatch(self):
        ...
        patches = status.collect_patches(series, 1234, self._FakePatchwork)
    ...
    def testFindNewResponses(self):
        ...
        self.assertEqual(
            terminal.PrintLine('    Tested-by: ', col.GREEN, newline=False,
                               bright=False),
            next(lines))
        self.assertEqual(terminal.PrintLine(self.ed, col.WHITE, bright=False),
                         next(lines))
        self.assertEqual(
            terminal.PrintLine('    Reviewed-by: ', col.GREEN, newline=False,
                               bright=False),
