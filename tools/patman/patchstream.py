...
class PatchStream:
    ...
    def ProcessLine(self, line):
        ...
        if series_tag_match or commit_tag_match or change_id_match or \
           cover_match or signoff_match or self.state == STATE_MSG_HEADER:
            ...
            if self.in_section:
                self.AddWarn("Missing 'END' in section '%s'" % self.in_section)
            ...
            if self.in_change:
                self.AddWarn("Missing 'blank line' in section '%s-changes'" %
                             self.in_change)
                self.FinalizeChange()
                self.in_change = None
        ...
        # Detect Commit-xxx tags
            ...
            if name == 'notes':
                ...
                self.AddWarn('Line %d: Ignoring Commit-%s' %
                    (self.linenum, name))
        ...
        # Well that means this is an ordinary line
            ...
            if m:
                self.AddWarn('Line %d/%d has space before tab' %
    ...
    def Finalize(self):
        ...
        if self.lines_after_test:
            self.AddWarn('Found %d lines after TEST=' % self.lines_after_test)
            ...
            for line in out:
                ...
                if line == '+':
                    ...
                    if self.blank_count and (line == '-- ' or match):
                        self.AddWarn("Found possible blank line(s) at end of file '%s'" %
                                     last_fname)
                    outfd.write('+\n' * self.blank_count)
                    outfd.write(line + '\n')
                    self.blank_count = 0
