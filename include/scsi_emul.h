 ...
 *
 * implementations of SCSI functions requried so
 ...
 *
 * This sets upt he response in info->buff and updates various other values
 * in info.
 *
 * If SCSI_EMUL_DO_READ is returned then the caller should set up so that that
 * the backing file can be read, or return an error status if there is no file.
 *
