 ...
 *	10 for address,
 *	two hex digits, an ASCII character and a space for each byte
 *	4 spaces between the hex and ASCII
 *	\0 terminator
 */
#define MAX_BUF_LENGTH			(10 + MAX_LINE_LENGTH_BYTES * 4 + 5)
		 ...
		 uint linelen, char *out, int size)
{
	...
	thislinelen = linelen;
	out += snprintf(out, end - out, "%08lx:", addr);
	...
	for (i = 0; i < thislinelen; i++) {
		...
		if (CONFIG_IS_ENABLED(USE_TINY_PRINTF))
			...
			out += snprintf(out, end - out, " %0*lx", width * 2, x);
		data += width;
	}
	...
	lb.uc[i] = '\0';
	out += snprintf(out, end - out, "    %s", lb.uc);
