...
static int scene_build(struct build_info *info, struct expo *exp)
{
	...
	ret = scene_menu(scn, "save", CEDIT_SAVE, &menu);
	if (ret < 0)
		return log_msg_ret("men", ret);
	menu_id = ret;
	...
	ret = scene_menu(scn, "nosave", CEDIT_DISCARD, &menu);
	if (ret < 0)
		return log_msg_ret("men", ret);
	menu_id = ret;
...
static int build_it(struct build_info *info, struct expo **expp)
{
	...
	info->priv = calloc(1, sizeof(struct cedit_priv));
	if (!info->priv)
		return log_msg_ret("pri", -ENOMEM);

	ret = expo_new("coreboot", info->priv, &exp);
	if (ret)
		return log_msg_ret("exp", ret);
	expo_set_dynamic_start(exp, CEDIT_FIRST_DYNAMIC);
