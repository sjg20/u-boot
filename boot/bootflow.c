...
static int iter_incr(struct bootflow_iter *iter)
{
	...
	if (iter->flags & BOOTFLOWF_SINGLE_DEV) {
		...
		log_debug("inc_dev=%d\n", inc_dev);
		if (!inc_dev) {
			ret = bootdev_setup_iter_order(iter, NULL, &dev,
						       &method_flags);
		} else if (iter->flags & BOOTFLOWF_SINGLE_UCLASS) {
			  ...
			  struct bootflow *bflow)
{
	...
	if (!IS_ENABLED(CONFIG_BOOTMETH_GLOBAL) || !iter->doing_global) {
		...
		ret = bootdev_setup_iter_order(iter, label, &dev,
					       &method_flags);
		if (ret)
