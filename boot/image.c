...
#if CONFIG(FIT) || CONFIG(OF_LIBFDT)
		 ...
		 uint unc_len, ulong *load_end)
{
	...
	*load_end = load;
	...
	case IH_COMP_NONE:
		...
		break;
	case IH_COMP_GZIP:
		if (!tools_build() && CONFIG(GZIP))
			ret = gunzip(load_buf, unc_len, image_buf, &image_len);
		break;
	case IH_COMP_BZIP2:
		if (!tools_build() && CONFIG(BZIP2)) {
			uint size = unc_len;
