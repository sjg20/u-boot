	...
	if ((le32_to_cpu(tpc->RxDescArray[cur_rx].status) & OWNbit) == 0) {
		if (!(le32_to_cpu(tpc->RxDescArray[cur_rx].status) & RxRES)) {
			...
			tpc->RxDescArray[cur_rx].buf_addr =
				cpu_to_le32(pci_mem_to_phys(bdf,
			(pci_addr_t)(unsigned long)tpc->RxBufferRing[cur_rx]));
...
static void rtl8169_hw_start(pci_dev_t bdf)
{
	...
	RTL_W32(TxDescStartAddrLow,
		pci_mem_to_phys(bdf, (pci_addr_t)(unsigned long)tpc->TxDescArray));
	RTL_W32(TxDescStartAddrHigh, (unsigned long)0);
	RTL_W32(RxDescStartAddrLow, pci_mem_to_phys(bdf, (pci_addr_t)(unsigned long)tpc->RxDescArray));
...
static void rtl8169_init_ring(pci_dev_t bdf)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		...
		tpc->RxDescArray[i].buf_addr =
			cpu_to_le32(pci_mem_to_phys(bdf, (pci_addr_t)(unsigned long)tpc->RxBufferRing[i]));
