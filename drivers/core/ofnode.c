...
void *ofnode_to_fdt(ofnode node)
{
	...
	if (of_live_active())
		return NULL;
#endif
	if (CONFIG(OFNODE_MULTI_TREE) && ofnode_valid(node))
...
int ofnode_to_offset(ofnode node)
{
	...
	if (of_live_active())
		return -1;
#endif
	if (CONFIG(OFNODE_MULTI_TREE) && node.of_offset >= 0)
...
ofnode noffset_to_ofnode(ofnode other_node, int of_offset)
{
	...
	if (of_live_active())
		node.np = NULL;
	else if (!CONFIG(OFNODE_MULTI_TREE) || of_offset < 0 ||
...
static ofnode ofnode_from_tree_offset(oftree tree, int offset)
{
	...
	if (CONFIG(OFNODE_MULTI_TREE) && offset >= 0) {
