...
int device_of_to_plat(struct udevice *dev)
{
	...
	if (drv->of_to_plat &&
	    (CONFIG(OF_PLATDATA) || dev_has_ofnode(dev))) {
...
static int device_get_dma_constraints(struct udevice *dev)
{
	...
	if (!CONFIG(DM_DMA) || !parent || !dev_has_ofnode(parent))
		return 0;
...
int device_probe(struct udevice *dev)
{
	...
	if (CONFIG(POWER_DOMAIN) && dev->parent &&
