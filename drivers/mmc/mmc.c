...
#if CONFIG(MMC_VERBOSE) || defined(DEBUG) || CONFIG_VAL(LOGLEVEL) >= LOGL_DEBUG
...
{
	if (CONFIG(MMC_QUIRKS) && mmc->quirks & quirk)
			      ...
			      bool hsdowngrade)
{
	...
	case MMC_DDR_52:
		...
		break;
#if CONFIG(MMC_HS200_SUPPORT)
	case MMC_HS_200:
		speed_bits = EXT_CSD_TIMING_HS200;
		break;
#endif
#if CONFIG(MMC_HS400_SUPPORT)
	case MMC_HS_400:
		speed_bits = EXT_CSD_TIMING_HS400;
		break;
#endif
#if CONFIG(MMC_HS400_ES_SUPPORT)
	case MMC_HS_400_ES:
		speed_bits = EXT_CSD_TIMING_HS400;
		break;
#endif
...
#if CONFIG(MMC_VERBOSE) || defined(DEBUG)
/*
...
{
#if CONFIG(DM_MMC) && CONFIG(DM_REGULATOR)
	if (mmc->vmmc_supply) {
