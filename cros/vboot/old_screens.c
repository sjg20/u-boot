...
/**
 * load_archive() - load a locale file from an archive
 *
 * @locale_name: Name of locale file to load
 * @dest: Returns information about the file
 * @return  0 if OK, -EINVAL if no locales could be found, -ENOENT if the
 *	requested file was not found, -ENOMEM if not enough memory, -EINVAL if
 *	the archive size does not match its header information, -EPERM if the
 *	archive's magic is wrong, -E2BIG if the archive file extends beyond the
 *	end of the archive
 */
static int load_archive(const char *locale_name, struct directory **dest)
{
	...
	abuf_init(&buf);
	ret = vbfile_load(vboot, locale_name, &buf);
	if (ret) {
		log_err("Cannot read locale '%s'\n", locale_name);
		return log_msg_ret("read", ret);
	}

	/* Take the data out of the abuf */
	dir = (struct directory *)abuf_uninit_move(&buf, &size);
	if (!dir)
		return log_msg_ret("move", -ENOMEM);
	...
	if (dir->size != size) {
		log_err("archive size %x does not match region size %zx\n",
			dir->size, size);
	...
	if (memcmp(dir->magic, CBAR_MAGIC, sizeof(CBAR_MAGIC))) {
		...
		return log_msg_ret("magic", -EPERM);
	}
...
static int vboot_init_locale(struct vboot_info *vboot)
{
	...
	if (ret) {
		ret = log_msg_ret("locales", ret);
		goto err;
	}

	/* Copy the file and null-terminate it */
	loc_start = malloc(abuf_size(&buf) + 1);
	if (!loc_start) {
		ret = log_msg_ret("locales", -ENOMEM);
		goto err;
	}
	size = abuf_size(&buf);
	memcpy(loc_start, abuf_data(&buf), size);
	loc_start[size] = '\0';
	abuf_uninit(&buf);
