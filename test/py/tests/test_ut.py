...
def mount_image(cons, fname, mnt):
    """Mount a disk image and mount the filesystem on partition 1
    ...
    loop = out.strip()
    part = f'{loop}p1'
    u_boot_utils.run_and_log(cons, f'sudo mkfs.vfat {part}')
    u_boot_utils.run_and_log(
        cons,
        f'sudo mount -o loop {part} {mnt} -o uid={os.getuid()},gid={os.getgid()}')
    return loop
...
def setup_bootmenu_image(cons):
    ...
    complete = False
    try:
        loop = mount_image(cons, fname, mnt)
        mounted = True
...
''' % (mmc_dev)
        ...
        with open(os.path.join(bootdir, 'boot.bmp'), 'wb') as outf:
