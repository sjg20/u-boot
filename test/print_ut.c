...
static int print_test_hexdump_line(struct unit_test_state *uts)
{
	char *linebuf;
	u8 *buf;
	int i;
...
PRINT_TEST(print_test_hexdump_line, UT_TESTF_CONSOLE_REC);

static int print_test_hex_dump(struct unit_test_state *uts)
{
	u8 *buf;
	int i;
...
PRINT_TEST(print_test_hex_dump, UT_TESTF_CONSOLE_REC);
