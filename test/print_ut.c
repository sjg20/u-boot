...
/* Test printing GUIDs */
static int print_test_guid(struct unit_test_state *uts)
{
...
PRINT_TEST(print_test_guid, 0);
#endif

#if CONFIG_IS_ENABLED(EFI_LOADER) && !defined(API_BUILD)
/* Test efi_loader specific printing */
static int print_test_efi_ut(struct unit_test_state *uts)
{
	char str[10];
...
PRINT_TEST(print_test_efi_ut, 0);
#endif

static int print_test_printf(struct unit_test_state *uts)
{
	char big_str[400];
	int big_str_len;
	char str[10], *s;
...
PRINT_TEST(print_test_printf, 0);

static int print_test_display_buffer(struct unit_test_state *uts)
{
	u8 *buf;
	int i;
...
PRINT_TEST(print_test_display_buffer, UT_TESTF_CONSOLE_REC);

int do_ut_print(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	struct unit_test *tests = ll_entry_start(struct unit_test, print_test);
	const int n_ents = ll_entry_count(struct unit_test, print_test);

	return cmd_ut_category("cmd_print", "print_test_", tests, n_ents, argc,
			       argv);
}
