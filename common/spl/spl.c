...
#if CONFIG(BINMAN_UBOOT_SYMBOLS)
...
#if defined(CONFIG_SPL_OS_BOOT) || CONFIG(HANDOFF) || \
...
static int spl_common_init(bool setup_malloc)
{
	...
	if (CONFIG(DM)) {
		...
		ret = dm_init_and_scan(!CONFIG(OF_PLATDATA));
...
#if defined(CONFIG_SPL_WATCHDOG) && CONFIG(WDT)
	initr_watchdog();
#endif
...
{
#if CONFIG(SYS_REPORT_STACK_F_USAGE)
