				  ...
				  const char **outname)
{
	...
	if (!found && CONFIG(SYSINFO) && !sysinfo_get(&sysinfo)) {
		int rc;
		/*
			      ...
			      const struct spl_fit_info *ctx)
{
	...
	if (CONFIG(FIT_IMAGE_TINY))
		return 0;

	if (CONFIG(LOAD_FIT_APPLY_OVERLAY)) {
		void *tmpbuffer = NULL;
...
{
	if (!CONFIG(FIT_IMAGE_TINY) || CONFIG(OS_BOOT))
